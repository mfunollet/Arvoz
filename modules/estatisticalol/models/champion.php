...
class Champion extends DataMapperExt {

    var $table = 'lol_champions';
    ...
    function get_general() {
        $data[0]['url'] = 'http://br.riot5.com/stats/na/' . $this->code;
        
        //$data[0]['opcoes'][CURLOPT_PROXY] = '10.4.1.10';
        //$data[0]['opcoes'][CURLOPT_PROXYPORT] = '8080';
        $data[0]['opcoes']['CURLOPT_REFERER'] = 'http://br.riot5.com/';
        //$data[0]['cookie'] = 'lol';
        $html = $this->CI->curl->get($data);
        $html = procurar('<div class="page_bordered">', $html, 'ShowMasteryPage($(this));');
        
        // Pegando dados misc
        $this->misc['name'] = trim(procurar('itemprop="name">', $html, '</h1>'));
        
        // Pegando Total Stats
        $tabela = procurar('>Total Stats</h2>', $html, '</table>');
        $this->total_stats['wins'] = trim(procurar('<td class="summary_data">', $tabela, '<'));
        $this->total_stats['kills'] = trim(procurar('<td class="summary_data">', $tabela, '<', 2));
        $this->total_stats['assists'] = trim(procurar('<td class="summary_data">', $tabela, '<', 3));
        $this->total_stats['leaves'] = trim(procurar('<td class="summary_data">', $tabela, '<', 4));
        $this->total_stats['exp'] = trim(procurar('<span class="statistics_data_descr">', $tabela, '/'));
        $this->total_stats['total_exp'] = trim(procurar('<span class="statistics_data_descr">' . $this->total_stats['exp'] . ' /', $tabela, '<'));
        $this->total_stats['level'] = trim(procurar('<td class="summary_label">Level ', $tabela, '<'));
        $this->total_stats['last_game_data'] = trim(procurar('<td class="summary_data">', $tabela, '<', 6));
        
        // Last Game Progress
        $tabela = procurar('">Last Game Progress</h2>', $html, '</table>');
        $this->last_game_stats['champ_kills'] = trim(procurar('<td class="summary_data">', $tabela, '<'));
        $this->last_game_stats['deaths'] = trim(procurar('<td class="summary_data">', $tabela, '<',2));
        $this->last_game_stats['gold'] = trim(procurar('<td class="summary_data">', $tabela, '<',3));
        $this->last_game_stats['minions_kills'] = trim(procurar('<td class="summary_data">', $tabela, '<',4));
        $this->last_game_stats['torres_kills'] = trim(procurar('<td class="summary_data">', $tabela, '<',5));
        
        // Matches History 
        $tabela = procurar('<div id="progress_games_chart_div"', $html, '<div id="tab_awards_page"');
        //echo $tabela;
        //$this->matches_history[]
        
        //print_r($this->total_stats);
        //print_r($this->last_game_stats);
        $info = array();
        $info['misc'] = $this->misc;
        $info['total_stats'] = $this->total_stats;
        $info['last_game_stats'] = $this->last_game_stats;
    ...
    function was_logout($html) {
        if (strpos($html, 'Forgot Password?')) {
            return FALSE;
        }
        return TRUE;
    }
