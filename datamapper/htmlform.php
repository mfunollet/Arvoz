...
class DMZ_HTMLForm {
    ...
    // this is the default template (view) to use for the individual custom rows
    var $custom_template = 'dmz_htmlform/custom';
    ...
    function render_row($object, $field, $type = NULL, $options = array(), $row_template = NULL) {
        ...
        if (!isset($object->validation[$field]) && (empty($type) || $type == 'section' || $type == 'custom' || $type == 'none')) {
            ...
            if($type == 'custom'){
                ...
                if($pos = strrpos($field, '_')){
                    $field_name = substr($field, 0, $pos);
                    $label = $this->_get_validation_info($object, $field_name, 'label');
                    $content = $this->_get_custom($object, $field_name, 'custom');
                }
            }
        ...
        if (is_null($row_template)) {
            if (empty($type)) {
                ...
                $row_template = $this->custom_template;
            } else {
                $row_template = $this->row_template;
            }
        }
    ...
    // --------------------------------------------------------------------------
    // Attempts to look up the field's
    function _get_custom($object, $field, $custom = NULL) {
        $custom = $this->_get_validation_info($object, $field, 'custom', NULL);
        return $custom;
    }
